export type IllustrationPoint = [number, number];
export type IllustrationPlayerKind = 'attacker' | 'defender' | 'goalie';
export type IllustrationPlayerRole = 'center' | 'right-wing' | 'left-wing' | 'right-defense' | 'left-defense' | 'goalie';
export interface IllustrationPlayer { id: string; kind: IllustrationPlayerKind; role?: IllustrationPlayerRole | null; position: IllustrationPoint; rotation: number; scale: number; }
export interface IllustrationPath { id: string; step: number; kind: 'pass' | 'move' | 'shot'; curve: boolean; followsWall: boolean; points: IllustrationPoint[]; label: IllustrationPoint; }
export interface IllustrationScene { slug: string; version: 1; rink: 'stiga-playoff-v1'; published: boolean; viewport: { x: number; y: number; width: number; height: number }; paths: IllustrationPath[]; players: IllustrationPlayer[]; puck: null; }

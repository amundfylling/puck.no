/**
 * Worker-safe validators — copy of the pure parts of mcp/src/lib/validate.js
 * (which mirrors functions/api/registrations.ts — keep all in sync!).
 */
...
export function assertDateText(date, parseNoDate) {
  if (!parseNoDate(date)) {
    throw new ValidationError(

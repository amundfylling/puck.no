...
functions/               # Cloudflare Pages Functions (TypeScript):
  ...
  api/admin/registrations.ts      # POST/PATCH/DELETE admin-managed entries
  ...
  api/admin/registrations.json.ts # GET full rows for the protected portal
  api/admin/registration-open.ts  # POST open/close registration — commits
                                  # frontmatter via GitHub API (GITHUB_TOKEN)
migrations/              # ordered D1 schema migrations (apply all pending)
...
media-originals/         # ORIGINAL images (git-ignored, local disk only)
  images/  galleries/
media-uploads/           # committed CMS/MCP image sources for cloud builds
  images/
public/
...
keeps the committed web variants and also optimizes committed CMS/MCP sources
from `media-uploads/images/`. New editor uploads must go there so Cloudflare
can see them; duplicate relative paths across the two source roots fail builds.
...
- Backend (`functions/`): parameterised D1 queries only, strict server-side
  ...
  club/country/world_ranking/ranking_points/ranking_value are ignored. The
  separate ITHF `Player_Value` is the input to the WR 2020 placement-points
  calculator; it is not the player's total ranking points. Duplicates are
  rejected per tournament:
  ranked players on `player_id` and unranked individuals on `lower(email)`
  (partial unique indexes, `migrations/0002_player_id.sql` +
  `0003_team_player_ids.sql`); teams are deduplicated atomically inside the
  INSERT itself (`WHERE NOT EXISTS`) — ranked teams on any of their ranking
  ids, structured unranked roster names and the lowercased team display name.
  Duplicate contact emails ARE allowed for teams (contact info is stored per
  team). Tournament slugs + team rules:
...
- Tournaments: `name`, `slug`, `date`, `location`, `prices`,
  ...
  the API via `tournament-config.json`), `playersPerTeam` (null = individual;
  otherwise the number of highest-rated roster members included in team
  seeding), `maxSubstitutes` (default `0`), and `registrationQuestions`
  (bilingual single-choice questions with bilingual options and a required
  flag; answers are admin-only). `rankingLevel` is optional and accepts
  `1-world`, `1-continental`, `2`–`6` or `10`. It enables the bilingual public
  WR 2020 placement-points table, calculated from the current entrant count
...
frontmatter above. Put a new committed cover source in `media-uploads/images/` and
...
and the English home page. For a team tournament set `playersPerTeam` and
`maxSubstitutes` (e.g. `2`/`0` for Duo-NM); leave `playersPerTeam` null for an
...
Put a new committed image source in `media-uploads/images/` (or, for the
local historical archive, `media-originals/images/`) and reference it as
`/media/images/<file>`. Gallery masters remain in
`media-originals/galleries/<slug>/`; register each file in

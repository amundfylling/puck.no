...
4. Create or edit `src/content/illustrations/<slug>.json`. Set `illustration` in the trick record to the same slug and keep `published: false` while the scene is being reviewed.
...
- Keep the legacy raster as fallback and `published: false` until human comparison approves the editable scene in the admin portal.

...
4. Prefer `/admin/illustrasjoner/` for tracing and direct manipulation. For agent-authored coordinate changes, edit JSON directly and use the editor's grid/raw JSON panel for inspection.
5. Use one path per numbered action. Keep steps consecutive from 1. Let the shared renderer control colours, widths, arrowheads, fonts, rink asset, and labels.
6. Run `npm run check:illustrations` and `npm run check`.
7. Run `npm run build`, open both the catalogue thumbnail and detail page at desktop and 375 px, and compare against the description or legacy reference. Check that arrowheads, labels, and the decisive endpoint remain visible inside the crop.
...
- Place labels near the start of their path without covering the player or a rail.

...
4. Place the active players and puck before drawing arrows. Use `attacker` for the player(s) performing the combination, `defender` for opponents, and `goalie` for the keeper. Assign roles when known from the trick description.
...
- Keep player anchors on their physical rod position. Rotate and scale the shared sprite; never embed a per-scene image or pivot.

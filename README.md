...
- **Schema:** ordered migrations in `migrations/`. `0004_registration_details.sql`
  ...
  `0009_blog_reactions.sql` stores anonymous blog reactions; apply it before
  deploying the reaction bar.
- **Endpoints:**
  ...
  - `GET|POST /api/reactions/{lang}/{slug}` — live emoji counts and one editable
    reaction per browser per post. A one-year, API-scoped cookie identifies the
    browser; D1 stores a per-post hash, not the cookie or contact details.
    Translated post pairs use the Norwegian post's reaction key. The API's
    valid post list is generated by `scripts/gen-blog-post-keys.mjs` in prebuild.
